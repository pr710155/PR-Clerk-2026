...
function render(){
 ...
 const isMC=(S.level&&S.level.id===3) || (Array.isArray(q.options)&&q.options.length>0);
  ...
  const qout=(expr,ans,extra,highlight,approach,shortcut,steps)=>{
    ...
    while(all.length<5){
      const v=Number.isFinite(numeric)
        ? String(numeric + (filler%2 ? filler : -filler))
        : String(filler);
      filler++;
      if(!seen.has(v)){seen.add(v);all.push(v);}
    }
    const options=sh(all).slice(0,5);
...
function advancedMissingSeries(){
  const type=R(1,6);
  const clean=(arr)=>arr.map(v=>typeof v==="number"?(Number.isInteger(v)?String(v):String(Number(v.toFixed(2)))):String(v));
  const out=(series,answer,approach,shortcut,steps,highlight)=>{
    const n=Number(answer);
    const candidates=[answer,n+2,n-2,n+5,n-5,n+10,n-10];
    const seen=new Set(),options=[];
    for(const v of candidates){
      const sv=String(v);
      if(!seen.has(sv) && sv!=="NaN"){seen.add(sv);options.push(sv);}
      if(options.length===5)break;
    }
    while(options.length<5){
      const sv=String(n+options.length+11);
      if(!seen.has(sv)){seen.add(sv);options.push(sv);}
    }
    const z=q(`Find the missing term: ${clean(series).join(", ")}`,answer,"","Missing Number Series","Hard",sh(options));
    z.coach={highlight,approach,shortcut,steps,quickMethods:["Check the operation pattern before calculating.","Verify the rule on both sides of the gap."]};
    return z;
  };
  ...
  if(type===1){
    ...
    return out(shown,ans,"Separate odd and even transitions, then continue each operation.","Check alternating multiplication/addition before using differences.",[`Odd transitions use ×2.`,`Even transitions add ${add}+step number.`,`The missing term is ${ans}.`],"The operations alternate; don't force one rule across the whole series.");
  }
  ...
  if(type===2){
    ...
    const idx=R(2,5),ans=z[idx],shown=z.slice();shown[idx]="?";
    return out(shown,ans,"Write the first differences and compare them with consecutive squares.","If differences grow quickly, test squares before second differences.",[`Differences begin ${k}×1², ${k}×2², ${k}×3²...`,`The required difference at the gap gives ${ans}.`],`Successive differences follow ${k}×1², ${k}×2², ${k}×3²...`);
  }
  ...
  if(type===3){
    ...
    const idx=R(2,5),ans=z[idx],shown=z.slice();shown[idx]="?";
    return out(shown,ans,"Inspect the multiplier from one transition to the next.","Try ×2−n, ×3−n, ×4−n... when terms rise sharply.",[`Multipliers are ×2, ×3, ×4...`,`Subtract ${n} after each multiplication.`,`Missing term = ${ans}.`],"The multiplier increases by 1 while the subtraction stays fixed.");
  }
  ...
  if(type===4){
    ...
    const idx=R(2,5),ans=z[idx],shown=z.slice();shown[idx]="?";
    return out(shown,ans,"Take differences and classify each increment as n² or n³.","When differences jump irregularly, test alternating square/cube additions.",["Differences alternate: 1², 2³, 3², 4³...",`Continue the alternating rule to get ${ans}.`],"The increments alternate between a square and a cube.");
  }
  ...
  if(type===5){
    ...
    const idx=R(2,5),ans=z[idx],shown=z.slice();shown[idx]="?";
    return out(shown,ans,"Separate odd-position and even-position terms, then continue each sequence.","If consecutive differences look messy, split odd and even positions.",["Odd-position terms follow one progression.","Even-position terms follow another progression.",`The missing term is ${ans}.`],"There are two interleaved sequences hidden in the single row.");
  }
  ...
  const idx=R(2,5),ans=z[idx],shown=z.slice();shown[idx]="?";
  return out(shown,ans,"Look for ×n followed by +n² rather than only differences.","For rapidly growing terms, test ×n ± n² patterns.",[`Step multipliers increase: ×2, ×3, ×4...`,`The adjustment is the square of the multiplier.`,`Missing term = ${ans}.`],"Each step combines a changing multiplier with the square of that multiplier.");
}
